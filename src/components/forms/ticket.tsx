...
export function TicketStatusChangeForm({
  ticketId,
  currentStatus,
  ...
  const handleTicketSubmit = async (form: FormData) => {
    ...
    try {
      const selected = form.get("status");
      if (!selected) {
        toast.error("ステータスを選択してください", { id: toaster });
        return;
      }
      const status = selected as TicketStatus;
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {statuses.map((status) => (
              <label
                key={status.key}
                className="flex items-center gap-3 cursor-pointer p-3 rounded border border-gray-300 hover:border-black transition"
              >
                <input
                  type="radio"
                  name="status"
                  value={status.key}
                  defaultChecked={status.key === currentStatus}
                  required
